...
def test_preselection(
    ...
    default = updatable & (
        (templ | stand | admin) | (is_running & (disp | app))
    )
    ...
    expected = {
        ...
        (
            "--targets",
            f"{TemplVM.name},{StandVM.name}",
        ): {TemplVM, StandVM},
        (
            "--targets",
            f"{TemplVM.name},{TemplVM.name}",
        ): EXIT.ERR_USAGE,
        (
            "--targets",
            TemplVM.name,
    ...
    for args, selected in expected.items():
        if isinstance(selected, int):
            ...
            feed = {
                vm.name: {"statuses": [FinalStatus.SUCCESS], "retcode": EXIT.OK}
                for vm in selected
            }
            if feed:
        ...
        retcode = main(
            ("--force-update", "--just-print-progress", *args), test_qapp
        )
        ...
        failed[args] = {
            key: value for key, value in failed[args].items() if value
        }
...
def test_selection(
    ...
    stale = qlinux & (
        domains["updates_available"][False]
        & (
    ...
    for args, selected in expected.items():
        if isinstance(selected, int):
            feed = {}
            expected_exit = selected
            monkeypatch.setattr(vmupdate, "preselect_targets", lambda *_: all)
        else:
            feed = {
                vm.name: {"statuses": [FinalStatus.SUCCESS], "retcode": EXIT.OK}
                for vm in selected
...
def test_restarting(
    ...
    expected = {
        (): {"halted": set(), "restarted": set(), "untouched": all},
        ("--no-apply",): {
            "halted": set(),
            "restarted": set(),
            "untouched": all,
    ...
    for args, selected in expected.items():
        ...
        halted = {
            vm for vm in all if vm.shutdown.called and not vm.start.called
        }
        restarted = {vm for vm in all if vm.shutdown.called and vm.start.called}
        untouched = {
            vm for vm in all if not vm.shutdown.called and not vm.start.called
        }
        failed[args]["unexpected restart"] = set(
            map(lambda vm: vm.name, restarted - selected["restarted"])
        )
        failed[args]["not restarted"] = set(
            map(lambda vm: vm.name, selected["restarted"] - restarted)
        )
        failed[args]["unexpected shutdown"] = set(
            map(lambda vm: vm.name, halted - selected["halted"])
        )
        failed[args]["not halted"] = set(
            map(lambda vm: vm.name, selected["halted"] - halted)
        )
        failed[args]["unexpected untouched"] = set(
            map(lambda vm: vm.name, untouched - selected["untouched"])
        )
        failed[args]["unexpected touched"] = set(
            map(lambda vm: vm.name, selected["untouched"] - untouched)
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            stat.SUCCESS,
            EXIT.OK,
            EXIT.ERR_VM_UNHANDLED,
            id="unhandled retcode",
        ),
        pytest.param(
            stat.SUCCESS,
            EXIT.ERR_VM,
